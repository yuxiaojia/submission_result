...
shift
# echo "Arg3: "$1"";
# export num_sched_gpu="$1"
# shift
# echo "Arg4: "$1"";
# export baseline_or_cpcoh="$1"
# shift
echo "Arg4: "$*"";
export benchmark_options="$*"
export LD_LIBRARY_PATH=/usr/lib
mkdir m5out.$output_dir
...
# build/VEGA_X86/gem5.opt configs/example/apu_se.py -n 3 --dgpu --gfx-version=gfx900 -c bin/pathfinder --options= "100000 100 20" --num-compute-units=60 --cu-per-sa=15 --num-gpu-complex=4 --reg-alloc-policy=dynamic --num-tccs=8 --bw-scalor=8 --num-dirs=64 --mem-size=16GB --mem-type=HBM_2000_4H_1x64 --vreg-file-size=16384 --sreg-file-size=800 --tcc-size=4MB --gpu-clock=1801MHz --ruby-clock=1000MHz --vrf_lm_bus_latency=6 --mem-req-latency=69 --mem-resp-latency=69 --mandatory_queue_latency=1 --max-cu-tokens=160 --max-coalesces-per-cycle=10 --sqc-size=16kB --scalar-mem-req-latency=28 --TCP_latency=4 --tcp-assoc=16 --tcp-num-banks=16 --TCC_latency=121 --tcc-assoc=16 --tagAccessLatency=1 --dataAccessLatency=1 --tcp-rp=RRIPRP --tcc-rp=RRIPRP

    build/VEGA_X86/gem5.opt -r -d m5out.$output_dir configs/example/apu_se.py
    -n 3 --dgpu --gfx-version=gfx900 -c bin/$benchmark_name "--options=\"$benchmark_options\"" --num-compute-units=60 \
    --cu-per-sa=15 --num-gpu-complex=4 --reg-alloc-policy=dynamic --num-tccs=8 --bw-scalor=8\
    --num-dirs=64 --mem-size=16GB --mem-type=HBM_2000_4H_1x64 --vreg-file-size=16384 \
    --sreg-file-size=800 --tcc-size=4MB --gpu-clock=1801MHz --ruby-clock=1000MHz --vrf_lm_bus_latency=6 \
